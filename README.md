...
These mjml tempalates live in `./ol-keycloak/oltheme/src/main/resources/theme/ol/email/mjml/`. There is a script `./scripts/compile-emails.sh` that compiles these templates into `./ol-keycloak/oltheme/src/main/resources/theme/ol/email/html/generated` as `.ftl` files. If you add a new mjml template, you'll need to add a new template in the `html/` directory that includes the generated one. See `email-test.ftl` for an example.
